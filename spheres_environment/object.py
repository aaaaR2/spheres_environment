...
class Object(dict):
    ...
    def object_properties(self):
        ...
        return [
            k for k in attributes if is_object_property(getattr(self.__class__, k, {}))
        ]
    ...
    def to_record(self, delimiter="_"):
        ...
        from_sequence = lambda k, v: {
            f"{k}{delimiter}{i}": vi for (i, vi) in enumerate(v)
        }

        # Define a conversion for mapping types.
        from_mapping = lambda k, v: {
            f"{k}{delimiter}{ki}": vi for (ki, vi) in v.items()
        }
        ...
        record = {
            ki: vi for k in self.object_properties for (ki, vi) in convert(k).items()
        }
